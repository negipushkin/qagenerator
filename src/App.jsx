...
export default function App() {
  ...
  // QA module state
  const [qaScreen, setQaScreen] = useState('input')
  ...
  const handleHandoff = useCallback((content) => {
  ...
  const handleQaGenerate = useCallback(async () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Sticky header with branding + tab nav */}
      <header className="sticky top-0 z-10 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-4">
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <span className="font-bold text-gray-900 text-sm hidden sm:block">Built by Pushkin</span>
          </div>

          <div className="flex-1">
            <TabNav activeModule={activeModule} onSwitch={handleModuleSwitch} />
          </div>

          <div className="flex items-center gap-2 flex-shrink-0">
            {activeModule === 'qa' && qaScreen === 'results' && (
              <button
      ...
      {/* PM Co-Pilot module */}
      {activeModule === 'prd' && (
        <PMCopilot onHandoff={handleHandoff} />
      )}
